...
class Individuo:
    ...
    def evaluar(self, valores_entrada):
        ...
        # DENTRO DE LA CLASE Individuo
    def _evaluar_recursivo(self, nodo_actual, valores_entrada):
        # Caso Base 1: Si el nodo es una variable (x o y)
        if nodo_actual.valor in ['x', 'y']:
            return valores_entrada[nodo_actual.valor]
        
        # Caso Base 2: Si el nodo es una constante numérica
        if isinstance(nodo_actual.valor, (int, float)):
            return nodo_actual.valor

        # Caso Recursivo: Si el nodo es una función
        resultados_hijos = [self._evaluar_recursivo(hijo, valores_entrada) for hijo in nodo_actual.hijos]

        if nodo_actual.valor == '+':
            return resultados_hijos[0] + resultados_hijos[1]
        elif nodo_actual.valor == '-':
            return resultados_hijos[0] - resultados_hijos[1]
        elif nodo_actual.valor == '*':
            return resultados_hijos[0] * resultados_hijos[1]
        elif nodo_actual.valor == 'div':
            # División protegida para evitar errores por cero
            numerador = resultados_hijos[0]
            denominador = resultados_hijos[1]
            if abs(denominador) < 1e-6: # Un número muy pequeño
                return 1.0 # Devolvemos un valor seguro
            else:
